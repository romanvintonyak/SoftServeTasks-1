...
public class PointsOnMap implements IPointsFinder {
	...
	// FIXME add more comments here. You can also write here mathematic
	// formula that you use. It will be more understandable what this method
	// does.
	private long calculateDistance(IPoint point1, IPoint point2) {
		return Math.round(AVERAGE_LENGTH_ONE_DEGREE
				* Math.acos(Math.sin(point1.getLatitudeOnRad()) * Math.sin(point2.getLatitudeOnRad())
						+ Math.cos(point1.getLatitudeOnRad()) * Math.cos(point2.getLatitudeOnRad())
						* Math.cos(point2.getLongitudeOnRad() - point1.getLongitudeOnRad())) * 180 / Math.PI);
	}
